...
def make_figure_s3(
    ...
    ax = df.filter(
        variable=VAR_STORAGE_REL_VRE,
    ).plot(color="model", fill_between=True)

    ax.set_title("Electricity storage to VRE capacity ratio in European net-zero scenarios")
    ax.set_ylabel("Electricity storage relative to VRE installed power capacity")
    ...
    if save_png or save_pdf or save_svg:
        save_figure(
            fig,
            SUPPLEMENTARY_OUTPUT_DIR / "Figure-S3",
            save_png=save_png,
            save_pdf=save_pdf,
            save_svg=save_svg,
        )
